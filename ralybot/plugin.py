...
from ralybot.event import Event
...
def find_hooks(parent, module):
    ...
    for name, func in module.__dict__.items():
        if hasattr(func, "_ralybot_hook"):
            # If it has the ralybot hook
            func_hooks = func._ralybot_hook
...
class PluginManager:
    """
    PluginManager is the core of Ralybot plugin loading.
    ...
    :type event_type_hooks: dict[ralybot.event.EventType, list[EventHook]]
    ...
    def __init__(self, bot):
        """
        Creates a new PluginManager. You generally only need to do this from inside ralybot.bot.Ralybot
        :type bot: ralybot.bot.Ralybot
        """
        self.bot = bot
    ...
    def load_plugin(self, path):
        ...
        Won't load any plugins that are listed in the "disabled_plugins" section of the config.
        ...
        try:
            ...
            if hasattr(plugin_module, "_ralybot_loaded"):
    ...
    def _sieve(self, sieve, event, hook):
        """
        :type sieve: ralybot.plugin.Hook
        :type event: ralybot.event.Event
        :type hook: ralybot.plugin.Hook
        :rtype: ralybot.event.Event
        """
        try:
            if sieve.threaded:
    ...
    def launch(self, hook, event):
        ...
        :type event: ralybot.event.Event | ralybot.event.CommandEvent
        :type hook: ralybot.plugin.Hook | ralybot.plugin.CommandHook
        :rtype: bool
        """
...
class CommandHook(Hook):
    ...
    def __init__(self, plugin, cmd_hook):
        """
        :type plugin: Plugin
        :type cmd_hook: ralybot.util.hook._CommandHook
        """
...
class RegexHook(Hook):
    ...
    def __init__(self, plugin, regex_hook):
        """
        :type plugin: Plugin
        :type regex_hook: ralybot.util.hook._RegexHook
        """
...
class PeriodicHook(Hook):
    ...
    def __init__(self, plugin, periodic_hook):
        """
        :type plugin: Plugin
        :type periodic_hook: ralybot.util.hook._PeriodicHook
        """
...
class RawHook(Hook):
    ...
    def __init__(self, plugin, irc_raw_hook):
        """
        :type plugin: Plugin
        :type irc_raw_hook: ralybot.util.hook._RawHook
        """
...
class SieveHook(Hook):
    def __init__(self, plugin, sieve_hook):
        """
        :type plugin: Plugin
        :type sieve_hook: ralybot.util.hook._SieveHook
        """
...
class EventHook(Hook):
    """
    :type types: set[ralybot.event.EventType]
    """

    def __init__(self, plugin, event_hook):
        """
        :type plugin: Plugin
        :type event_hook: ralybot.util.hook._EventHook
        """
...
class OnStartHook(Hook):
    def __init__(self, plugin, on_start_hook):
        """
        :type plugin: Plugin
        :type on_start_hook: ralybot.util.hook._On_startHook
        """
