...
# import bot
from ralybot.bot import Ralybot
...
def main():
    ...
    # Ralybot.run() will return True if it should restart, False otherwise
    restart = _bot.run()

    # the bot has stopped, do we want to restart?
    if restart:
        # remove reference to ralybot, so exit_gracefully won't try to stop it
        _bot = None
        ...
        if stopped_while_restarting:
            ...
            args = sys.argv
            logger.info("Restarting Ralybot.")
