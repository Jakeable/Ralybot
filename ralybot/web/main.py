...
def get_template_env():
    env = Environment(loader=PackageLoader('ralybot.web'))
    return env
...
def get_application():
    app = Application([
        ...
        (r"/s/(.*)", StaticFileHandler, {"path": "./ralybot/web/static"}),
...
class TestHandler(RequestHandler):
    ...
    def get(self):
        ...
        args = {
            'bot_name': wi.config.get('bot_name', 'Ralybot'),
            'bot_version': ralybot.__version__,
...
class StatusHandler(RequestHandler):
    ...
    def get(self):
        ...
        for conn in wi.bot.connections:
            pass
        args = {
            'bot_name': wi.config.get('bot_name', 'Raly  Butt'),
            'bot_version': ralybot.__version__
        }
        self.write(template.render(**args))
...
class WebInterface():
    ...
    def start(self):
        """ Starts the Ralybot web application """
