...
class _EventHook(_Hook):
    ...
    def add_hook(self, trigger_param, kwargs):
        """
        :type trigger_param: ralybot.event.EventType | list[ralybot.event.EventType]
        :type kwargs: dict[str, unknown]
        """
        self._add_hook(kwargs)
...
def _add_hook(func, hook):
    if not hasattr(func, "_ralybot_hook"):
        func._ralybot_hook = {}
    else:
        assert hook.type not in func._ralybot_hook  # in this case the hook should be using the add_hook method
    func._ralybot_hook[hook.type] = hook


def _get_hook(func, hook_type):
    if hasattr(func, "_ralybot_hook") and hook_type in func._ralybot_hook:
        return func._ralybot_hook[hook_type]
...
def event(types_param, **kwargs):
    ...
    :type types_param: ralybot.event.EventType | list[ralybot.event.EventType]
    """
